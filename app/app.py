...
def status():
    ...
    # Analyse du dernier backup
    backup_files = glob.glob(os.path.join(BACKUP_DIR, "app-*.db"))
    if not backup_files:
        return jsonify(count=n, last_backup_file=None, backup_age_seconds=None)

    latest_backup = max(backup_files, key=os.path.getmtime)
    file_name = os.path.basename(latest_backup)
    age = int(time.time() - os.path.getmtime(latest_backup))
